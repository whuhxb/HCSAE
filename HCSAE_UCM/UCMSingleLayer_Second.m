...
numpatches = 1680*patchonepic;
patches = zeros(patchDim*patchDim*3, numpatches);
IMAGES=reshape(colorIMAGES,256*256*3,100,21);
IMAGES1=zeros(256*256*3,80,21);
% Generate fixed random number for traing the first and second layer
A=cell(21,1);
for i=1:21
randnum=randperm(100);
A{i}=randnum;
IMAGES1(:,:,i)=IMAGES(:,randnum(1:80),i);
end
save A A;
IMAGES2=reshape(IMAGES1,256,256,3,80*21);
